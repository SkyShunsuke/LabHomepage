...
type AwardGroup = "hasegawa" | "students";
...
type AwardsBrowserProps = {
  items: AwardItem[];
  messages: AwardsBrowserMessages;
};
...
export function AwardsBrowser({ items, messages }: AwardsBrowserProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedGroup, setSelectedGroup] = useState<"all" | AwardGroup>("all");
        ...
        <div className="publication-controls-grid">
          ...
          <label>
            ...
            <select
              value={selectedGroup}
              onChange={(event) => {
                setSelectedGroup(event.target.value as "all" | AwardGroup);
                setCurrentPage(1);
              }}
            >
          ...
          <label>
            ...
            >
              ...
              {yearOptions.map((year) => (
                <option key={`award-year-${year}`} value={String(year)}>
                  {year}
                </option>
              ))}
            </select>
          ...
          <ul className="achievement-record-list">
            {pageItems.map((item) => (
              <li key={item.id} className="achievement-record-item">
                ...
                {item.authors ? <p className="muted achievement-record-meta">{item.authors}</p> : null}
